...
public class Parser {
 ...
 public static int line_count = 1; //Keep track of the line count
 public static Object error_backtrack; //Backtrack the previous token that caused error


 public static final boolean DebugFlag = false; //For Debug puporses (True to turn on)
 public static boolean standerdErrorOverride = true; //Default error print out
 ...
 public void Advance() throws java.io.IOException {
    ...
    line_count = _lexer.lineno;
    if (DebugFlag) System.out.println(line_count);
 ...
 public void parse() throws java.io.IOException, Exception {
    ...
    if (successparse)
     ...
     if (standerdErrorOverride) System.out.println("Syntax error: There is a syntax error at line " + (line_count) + ".");
     System.out.println("Error: there exists syntax error(s).");
    }
 }

 //Grammar Start from here
 //_token.type is defined value of the token
 ...
 public boolean fun_decl() throws java.io.IOException, Exception {
    ...
    switch (_token.type) {
     ...
     case INT:
        ...
        if (Match(IDENT) == false) {
         standerdErrorOverride = false;
         System.out.println("Syntax error: an identifier is expected instead of \"" + error_backtrack + "\" at line " + line_count + ".");
         return false;
        }

        if (DebugFlag) System.out.println(" ->fun_decl " + "\t\t\tIDENT Matched");
        if (Match(LPAREN) == false) {
         standerdErrorOverride = false;
         System.out.println("Syntax error: \"(\" is expected instead of \"" + error_backtrack + "\" at line " + line_count + ".");
         return false;
        }
    ...
    switch (_token.type) {
     case IDENT:
        ...
        if (Match(ASSIGN) == false) {
         standerdErrorOverride = false;
         System.out.println("Syntax error: \"=\" is expected instead of \"" + error_backtrack + "\" at line " + (line_count - 1) + ".");
         return false;
 ...
 public boolean compound_stmt() throws java.io.IOException, Exception {
    ...
    if (Match(BEGIN) == false) {
     standerdErrorOverride = false;
     System.out.println("Syntax error: There is a syntax error at line " + (line_count - 1) + "."); //BK
     return false;
    }
 ...
 public boolean if_stmt() throws java.io.IOException, Exception {
    ...
    switch (_token.type) {
     case IF:
        ...
        if (DebugFlag) System.out.println(" ->if_stmt " + "\t\t\t) Matched");
        if (stmt() == false) return false;
        int else_BK = line_count;
        if (Match(ELSE) == false) {
         standerdErrorOverride = false;
         System.out.println("Syntax error: \"else\" is expected instead of \"" + error_backtrack + "\" at line " + else_BK + ".");
         return false;
        }
